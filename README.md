...
The `games/boringjob/` package wires a Sweet-Bonanza-style scatter game with on-board multiplier symbols and seven bet modes:
...
- `no_small_bomb` (cost 500.0, buybonus, trims the smallest multiplier tiers)
- `min_one_x10` (cost 5.0, guarantees at least one 10x+ multiplier per spin)
- `min_one_x100` (cost 250.0, guarantees at least one 100x+ multiplier per spin)
- `min_one_x1000` (cost 1000.0, guarantees at least one 1000x+ multiplier per spin)

Multipliers (`B`) are applied after the tumble chain ends for a spin, multiplying the accumulated tumble win. Mode constraints are enforced during board generation (not filtered post-hoc) by injecting or selecting `B` symbols that meet the required multiplier floor; the `no_small_bomb` mode removes sub-10x tiers from its multiplier tables, and the `doubleboost` mode increases the quota of forced free-game spins to double the bonus-entry likelihood.
